...
import sqlite3
import re
from bot import selectbd as slbs
...
def start_the_parser():
    ...
    for i in range(1,25,1) :
        ...
        url = 'https://freelance.ru/project/search?page='+str(i)+'&per-page=25'
        r =requests.get(url)
        src = r.text
        ...
        key_words = slbs.sbor_key_words()
        for itemm in key_words:
            ...
            for el,elb,eld,ele in zip(a,b,d,link) :#
                ...
                elb = ' '.join(elb)
                result = re.search(str(itemm), el)# ИЩЕМ В EL(ЗАГОЛОВКЕ) СЛОВО бот
                if result !=None :
                    c+=1
...
#         # ЭТОТ КОД НЕОБХОДИМ ДЛЯ СКАЧКИ HTML ДОКУМЕНТА СТРАНИИЦЫ , ЧТОБЫ НЕ ДОЛБИТЬ САЙТ
#         url = 'https://freelance.habr.com/tasks?_=1631304026186&categories=development_bots&page=' + str(i)
#         r = requests.get(url)
#         src = r.text
#
#         # ЭТОТ КОД НЕОБХОДИМ ДЛЯ СКАЧКИ HTML ДОКУМЕНТА СТРАНИИЦЫ , ЧТОБЫ НЕ ДОЛБИТЬ САЙТ
